...
export class ThreadService {
    ...
    async getThreadsWithPagination(skip: string, take: string, search: string, order_by: string, order: string, user_id: string) {
        try {
            let filter_object = {};
            // if got search get add a or where clause in the filter ie if searched term is in either heading or content.
            if (search && search.length) {
                filter_object["where"] = {
                    OR: [{ heading: { contains: search } }, { content: { contains: search } }]
                }
            }
            // if given a valid field with to order by sort the result in correct order.
            ...
            // getting the number of records with any additional query.
            let threads_count = await this.prisma.thread.findMany(filter_object);

            // adding pagination and including the user liked/disliked the thread. 
            filter_object = {
                ...filter_object,
